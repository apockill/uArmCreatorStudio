...
String runCommand(String cmnd){
    ...
    if(cmnd.indexOf(F("ss")) >= 0){
       ...
       if(      values[0] == SERVO_ROT_NUM){
         uarm.g_servo_rot.write(values[1]);
         uarm.cur_rot = values[1];
       }else if(values[0] == SERVO_LEFT_NUM){    
         uarm.g_servo_left.write(values[1]);
         uarm.cur_left = values[1];
       }else if(values[0] == SERVO_RIGHT_NUM){   
         uarm.g_servo_right.write(values[1]);
         uarm.cur_right = values[1];
       }else if(values[0] == SERVO_HAND_ROT_NUM){
         uarm.g_servo_hand_rot.write(values[1]);
         uarm.cur_hand = values[1];
         currentHand = values[1]; //uarm.inputToReal(SERVO_HAND_ROT_NUM, values[0]);
       }
       return ok;
    }else
